...
export class SaucelabsDaemon {
  ...
  /* Have the browsers been launched already (or in the process of being launched) */
  private _launching: boolean = false;
  private _launched: boolean = false;
  private _launchError: Error | undefined = undefined;
  ...
  async launchBrowsers() {
    if (this._launching) {
      // Looks like we've already done this
      return
    }
  ...
  async startTest(test: BrowserTest): Promise<boolean> {
    if (!this._launching) {
      // If this is the first test to run and we haven't launched any browsers yet
      // then its now or never
      await this.launchBrowsers().catch(err => {
        this._launchError = err;
        throw err
      })
    }

    while (!this._launched) {
      // We might not have been the request that launched the browsers so we
      // have to wait around until they are ready to go
      await new Promise(r => setTimeout(r, 250))
      if (this._launchError) {
        // Launch failed
        throw this._launchError
      }
    }
